...
# Solicitar al usuario el valor del ticker y las fechas
...
def insertVaribleIntoTable( ticker  , date , open, high , low , close, volume , start_date , end_date ):
    try:
        conn = sqlite3.connect(db)
        cursor = conn.cursor()
        #print("Connected to SQLite")

        #** Agregar que valide que si el dato a insertar existe no lo inserte **#
        #
        #               TOMAS ***************************************************
        #************************************************************************
        sqlite_insert_with_param = """INSERT INTO base_datos_stock
                          (ticker  , date , open, high , low , close, volume , start_date , end_date)
                          VALUES (?, ?, ?, ?, ? , ?, ? ,?,?  );"""

        data_tuple = ( ticker  , date , open, high , low , close, volume , start_date , end_date )
        cursor.execute(sqlite_insert_with_param, data_tuple)
        conn.commit()
        #print("Python Variables inserted successfully into base_datos_stock table")
    ...
    if data['queryCount'] == 0:
        ...
        for i in range(len(a)):
            ...
            for j in range(len(b)):
                insertVaribleIntoTable( ticker , datetime.date.fromtimestamp(b[j].get('t')/1000.00) ,float(b[j].get('o'))  ,float(b[j].get('h')),float(b[j].get('l')) ,float(b[j].get('c')) ,float(b[j].get('v')), str(fecha_inicio) , str(fecha_fin))
else:
